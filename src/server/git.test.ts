...
describe('featureHistory', () => {
  ...
  it('includes a tag created by a later release commit', async () => {
    ...
    const history = await featureHistory(repo, file)
    expect(history.commits[0]?.release).toBe('v1.0.0')
  })

  it('uses the earliest of several tags containing a commit', async () => {
    const file = path.join(repo, '.chocks', 'auth.chocks.md')
    await writeFile(file, '---\ntitle: Auth\n---\n', 'utf8')
    await commit('feat: add auth')
    ...
    expect((await featureHistory(repo, file)).commits[0]?.release).toBe('v1.0.0')
  })

  it('omits release when no tag contains a commit', async () => {
    const file = path.join(repo, '.chocks', 'auth.chocks.md')
    await writeFile(file, '---\ntitle: Auth\n---\n', 'utf8')
    await commit('feat: add auth')
    expect((await featureHistory(repo, file)).commits[0]?.release).toBeUndefined()
  })
