...
`code` is a separate, similarly shaped list claiming where a feature is implemented. Where a link is somewhere to click, a `code` entry is a repo-relative glob that a check could confirm, which is why chocks doesn't try to make it clickable. Each entry needs a `path`; an optional `kind` is one of `code`, `test` or `flag` and defaults to `code` — an unrecognised `kind` falls back to the default rather than being dropped. It's capped at 20 entries the same way `links` is. Nothing verifies these paths exist or match anything yet; `code` is read-only in the UI, and is only the input a future `chocks audit` would check.
