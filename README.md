...
`links` is an ordered list of up to 20 objects. Each needs a `url`. HTTP, HTTPS and protocol-relative URLs are clickable; repo-relative paths and other schemes render as plain text because chocks does not serve files from the repo. An optional `label` replaces the URL as the displayed text. An optional `type` adds an icon for `docs`, `issue`, `pr`, `design` or `spec`; anything else gets the generic link icon without being corrected or dropped. A hand-written file with more than 20 entries opens with the first 20, while an API write over the limit is rejected. Links are read-only in the UI for now, so editing one is a hand edit.
